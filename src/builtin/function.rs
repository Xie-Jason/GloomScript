        ...
        BuiltinClass {
            ...
            get_ref_type_fn: Box::new(|option| {
                if let Some(mut generic) = option {
                    if generic.len() == 0 {
                        Result::Ok(RefType::Func(Box::new((
                            Vec::new(),
                            ReturnType::Void,
                            false,
                        ))))
                        ...
                        let param_types = match generic_type {
                            DataType::Ref(RefType::Tuple(vec_box)) => *vec_box,
                        ...
                        if generic_return_type.is_none() {
                            ...
                            Result::Ok(RefType::Func(Box::new((
                                param_types,
                                ReturnType::Have(generic_return_type),
                                false,
                            ))))
                        }
                    } else {
                        Result::Err(format!(
                            "unexpect generic type {:?} of Func, expect 1 or 2 generic type parameter",
                            generic
                        ))
                    }
                } else {
